...
## Examiner Packaging Guidance

I will provide a zip containing only the files required to run locally: `run.py`, `streamlit_main.py`, `src/` (analytics/core modules used by UI), `requirements.txt`, minimal `data/` artifacts (SQLite if needed), and `README.md`. Everything else (tests, notebooks, heavy logs) will be excluded unless requested.
